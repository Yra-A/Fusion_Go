...
func ContestFavoriteAction(user_id int32, contest_id int32, action_type int32) error {
    ...
    if action_type == 1 {
        ...
        if errors.Is(err, gorm.ErrRecordNotFound) {
            ...
            // 异步更新 cache
            go func() {
                rdFav.AddFavoriteContest(int64(user_id), int64(contest_id))
            }()
            return err
        }
...
func FetchContestFavoriteList(user_id int32, limit int32, offset int32) ([]int32, error) {
    ...
    go func(contestIds []int32) {
        for _, x := range contestIds {
            rdFav.AddFavoriteContest(int64(user_id), int64(x))
        }
    }(contestIds)
